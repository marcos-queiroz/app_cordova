window.mydb =  {
    ...
    DB_NAME: "DB_bee_Financeiro",
    DB_VERSION: "1.0",
    DB_DESCRIPTION: "BE do APP bee Financeiro",
    DB_SIZE: 15*1024*1024, // 15 MB
    DB_TABLES: [],
    ...
    replace: function(table, jsonRegister, callback){
        var self = this;
        this.db.transaction(function(tx) {
            var columns = [];
            var values = [];
    ...
    ExecutaSQL: function(sql, callback) {
        this.db.transaction( function(tx) {
            tx.executeSql(sql, [], function (tx, results) {
                var len = results.rows.length;
                var registers = [];
